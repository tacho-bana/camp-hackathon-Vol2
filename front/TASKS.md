...
- [x] `utils/geo.ts` を作成する（Haversine 公式で距離計算する `calcDistance(a: LatLng, b: LatLng): number`）
- [x] `hooks/useNearbyPOI.ts` を作成する（Mapbox Search API で現在地周辺の POI を取得）
- [x] `MapView` の `nearbyPlaces` マーカーを実際の緯度経度でプロット（モック座標から移行）
- [x] 現在地から各 POI までの距離を `calcDistance` で計算して `NearbyPlace.distance` に反映する
- [x] `MapPage` のダミー `nearbyPlaces` 配列を `useNearbyPOI` の結果に差し替える
